...
<form action="{{ route('admin.editors.update', $editor) }}" method="POST" enctype="multipart/form-data" class="w-full">
    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-4">
        ...
        <div>
            ...
            <input type="number" name="legacy_user_id" min="1" value="{{ old('legacy_user_id', $editor->legacy_user_id) }}" class="w-full border rounded-lg px-4 py-2 @error('legacy_user_id') border-red-500 @enderror" placeholder="arsiv.rehagazetesi kullanıcı id">
        ...
        <div>
            ...
            <input type="number" name="editor_order" min="1" max="9999" value="{{ old('editor_order', $editor->editor_order) }}" class="w-full border rounded-lg px-4 py-2 @error('editor_order') border-red-500 @enderror" placeholder="Örn: 1, 2, 3... (boşsa otomatik)">
            @error('editor_order')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" id="can_access_archive" name="can_access_archive" value="1" @checked(old('can_access_archive', $editor->can_access_archive)) class="rounded border-gray-300 text-red-600 focus:ring-red-500">
            <label for="can_access_archive" class="text-sm text-gray-700">Arşiv yazılarına erişebilsin</label>
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" id="is_chief_columnist" name="is_chief_columnist" value="1" @checked(old('is_chief_columnist', $editor->is_chief_columnist)) class="rounded border-gray-300 text-red-600 focus:ring-red-500">
